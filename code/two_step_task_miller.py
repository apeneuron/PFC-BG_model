...
class Two_step():
    ...
    def __init__(self, common_prob=0.8, good_prob=0.8, rev_prob = 0.02, min_len = 10):
        super(Two_step, self).__init__()
        self.common_prob  = common_prob
        self.good_prob = good_prob
        self.rev_prob = rev_prob
    ...
    def step(self, action):
        ...
        if self.state in (initiate, reward_A, reward_B):
            if action == initiate: # Initate action.
                ...
                if (self.block_trial > self.min_len) and (np.random.rand()<self.rev_prob):
                    self.block_n += 1
